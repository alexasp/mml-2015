...
where the term $\mu\|\theta\|^{2}_{2}$ \todo{Shouldn't this be lambda?} is a regularization term to restrict the magnitude of $\theta$ and avoid overfitting the training data. Using $\mu > 0$ gives a “regularized” estimate of w which often has superior generalization performance, especially when the dimensionality is high (Nigam et al., 1999). 
...
where $\lambda$ is the regularization constant, $t$ is the current epoch and $\eta$ \todo{eta is generally used to indicate noise vector, so can we change this?} is a parameter used to tune the adaptive learning rate. Note that this means that we do not need to search for suitable values of the learning rate $\alpha$ - instead, we must identify a suitable $\eta$.
