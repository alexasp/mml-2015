...
\begin{definition}A mechanism $M$ is a random function that takes a dataset D as input, and outputs a random variable $M$(D).
\end{definition}
...
\Delta \mathit{f}=\underset{D_1,D_2}{max}||\mathit{f(D_1)}-\mathit{f(D_2)}||_{1}
 \end{eqnarray}
 \missingfigure[figwidth=6cm]{Picture showing the laplacian distribution} \unsure{Not sure if we need the pic}]
