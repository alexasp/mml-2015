...
S(u_f) = \underset{d(D_1,D_2)=1,a\in A}{max}||u_f(D_1, a)-u_f(D_2,a)||
 ...
 p(y = ±1|\textbf{x}, \theta) = \frac{1}{1 + exp( - \theta^T \textbf{x})}
  \end{eqnarray}
  where $\theta$ is the parameter vector we wish to learn. Following this we want to find $\theta$ such that:
  \begin{eqnarray}
  \theta = argmax_\theta \sum_{i=1}^{m} log Pr(y_i | x_i,\theta) - \mu \|\theta\|^{2}_{2}
    \end{eqnarray}
 
 where the term $\mu\|\theta\|^{2}_{2}$ is a regularization term to restrict the magnitude of $\theta$ and avoid overfitting the training data.
 It can be used for binary classification or for predicting the certainty of a binary outcome. See Cox  and Snell (1970) for the use of this model in statistics. This note focuses only on computational issues related to maximum-likelihood or more generally maximum a-posteriori (MAP) estimation. A common
...
\todo{Write about how this does not consider the fact that it might be necessary to protect ALL the records of a particular participant.}
