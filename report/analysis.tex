...
\begin{table}[h]
	\begin{tabular}{ll}
		...
		Sharma\cite{sharma2013adaptive}                                    & 0.0705 \\
		Kumar\cite{kumar2012comparative}                                  & 0.1389 \\
		Our optimal result ($\epsilon$=10, 1 peer)                        & 0.130  \\
		Less noise, less peers($\epsilon$=0.1, 10 peers)				  & 0.138  \\
		Less noise, more peers($\epsilon$=1.0, 100 peers)				  & 0.164  \\
		Noisy result ($\epsilon$=0.1, 50 peers)                           & 0.220  \\
		
		
	\end{tabular}
	\label{tab:baseline_class_results}
\end{table}
...
\todo[]{Explain what cold start issues is. Discuss how this is very prominent in our sitaution, and the different ways we can deal with the cold start, and pros and cons of those approaches.}
