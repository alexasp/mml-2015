...
\todo{insert figure showing situations where smaller groups is better}
\subsection{Analysis of Propagation and group size}

As mentioned in Section \ref{sec:PropagationPubModel}, 
...
To determine whether there is value in aggregation, it would be necessary to obtain a data set which has subsets that are produced by distinct distributions, and partitioning data without fully shuffling first. Due to time constraints we did not have time to run these additional experiments. \todo{See if we have time to do this. We REALLY should, otherwise it will very possibly give a lower grade.}
